    ...
    $wporg_atts = shortcode_atts(
		...
		$products = new WP_Query( $args );
			foreach ($products->posts as $product){
				...
				$grid_cards .= '<h1>'.get_post_meta( $product->ID, '_product_attributes')[0]['dormitorio']['value'].'H</h1></div>';
				$grid_cards .= '<div class="card-img-icon"><img src="http://www.tearriendo.cl/wp-content/uploads/2020/06/icono-ducha-05.png">';
				$grid_cards .= '<h1>'.get_post_meta( $product->ID, '_product_attributes')[0]['bano']['value'].'B</h1></div></div>';
				$grid_cards .= '<a class="card-footer" href='.get_permalink( $product->ID ).'>';
